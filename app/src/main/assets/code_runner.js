...
int main() {
  ...
  return 0;
            ...
            const compileResult = await executeTerminalCommand(`gcc -O3 -march=native -fopenmp ${tempCFile} -o ${tempCExec}`, 30000);
            if (compileResult.exitCode !== 0 || hasError(compileResult.output)) {
...
int main() {
  ...
  return 0;
            ...
            const compileResult = await executeTerminalCommand(`g++ -O3 -march=native -fopenmp ${tempCppFile} -o ${tempCppExec}`, 30000);
            if (compileResult.exitCode !== 0 || hasError(compileResult.output)) {
...
[dependencies]
    ...
    async function run_c(params) {
        ...
        try {
            await executeTerminalCommand(`cat <<'EOF' > ${tempFilePath}\n${script}\nEOF`);
            const compileResult = await executeTerminalCommand(`gcc -O3 -march=native -fopenmp ${tempFilePath} -o ${tempExecPath}`, 30000);
            if (compileResult.exitCode !== 0 || hasError(compileResult.output)) {
    ...
    async function run_c_file(params) {
        ...
        try {
            const compileResult = await executeTerminalCommand(`gcc -O3 -march=native -fopenmp ${filePath} -o ${tempExecPath}`, 30000);
            if (compileResult.exitCode !== 0 || hasError(compileResult.output)) {
    ...
    async function run_cpp(params) {
        ...
        try {
            await executeTerminalCommand(`cat <<'EOF' > ${tempFilePath}\n${script}\nEOF`);
            const compileResult = await executeTerminalCommand(`g++ -O3 -march=native -fopenmp ${tempFilePath} -o ${tempExecPath}`, 30000);
            if (compileResult.exitCode !== 0 || hasError(compileResult.output)) {
                throw new Error(`C++ 代码编译失败:\n${compileResult.output}`);
            }
            const result = await executeTerminalCommand(tempExecPath, 30000);
    ...
    async function run_cpp_file(params) {
        ...
        try {
            const compileResult = await executeTerminalCommand(`g++ -O3 -march=native -fopenmp ${filePath} -o ${tempExecPath}`, 30000);
            if (compileResult.exitCode !== 0 || hasError(compileResult.output)) {
                throw new Error(`C++ 文件编译失败:\n${compileResult.output}`);
            }
            const result = await executeTerminalCommand(tempExecPath, 30000);
